...
def test_causal_dwconv_fwd(B, T, d, k):
    ...
    tri = causal_dwconv_fwd(x_dt, W_conv, T, k, BLOCK_T)
    ref = F.conv1d(
        F.pad(x.float().transpose(1, 2), (k - 1, 0)), W_conv.unsqueeze(1), groups=d
    )

    assert torch.allclose(tri, ref, atol=1e-4), (
...
def test_rmsnorm_fwd(n_rows, d):
    ...
    W_norm = torch.ones(d, device=DEVICE)
    H_NORM, RSTD = rmsnorm_fwd(H, W_norm, 1e-6, BLOCK_D=d)
    rms = H.pow(2).mean(-1, keepdim=True).add(1e-6).sqrt()
    assert torch.allclose(H_NORM, H / rms * W_norm, atol=1e-5)
    assert torch.allclose(RSTD, (1.0 / rms).squeeze(-1), atol=1e-5)
...
def test_gate_stream_update_fwd(n_rows, d):
    ...
    side = torch.randn(n_rows, d, dtype=torch.bfloat16, device=DEVICE)
...
def test_fwd(B, T, d, k):
    ...
    assert torch.allclose(ref_fo.float(), tri_fo.float(), rtol=1e-2, atol=1e-2)
    assert torch.allclose(ref_so.float(), tri_so.float(), rtol=1e-2, atol=1e-2)
...
def test_bwd(B, T, d, k):
    ...
    for name, rg, tg in zip(names, ref_grads, tri_grads):
        assert torch.allclose(rg.float(), tg.float(), atol=1e-2), (
            f"d{name} max diff {(rg - tg).abs().max():.2e}"
        )
